...
* There are some issues on IE11. Homepage does not render properly. Some elements overlap and the footer is not at the bottom of the page. Cards height is longer than expected. Also, the questions on the beauty test page are squeezed to the middle, and payment form does not render. I tried to use autoprefixer on the css files, however it did not help and the issue persists. I tried to look for the root cause, however I could not find it. I suspect that there might be some Bootstrap classes that do not work as expected with IE, however IE inspect functions do not allow for in-depth analysis (unlike Chrome Dev tools). For this reason, and also because IE is no longer recommended to use, I decided not to investigate further, as it would bring little value. I thought of handling this error in a way that I hide the page content of the website completely, and only show a message that the application does not support IE, however this would mean I would have to wrap everything, including navbar and footer in one html element, and this did not seem like a good practice and use of semantic html. I also tried to place a div that would cover the whole screen but this did not work out either, so I decided not to handle this further, due to limited resources and the fact the IE is a no longer recommended browser, even by Microsoft. 
